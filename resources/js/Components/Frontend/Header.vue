...
<template>
    <nav class="fixed z-20 w-full transition duration-500 bg-light-primary border-gray-200 dark:bg-dark-primary" :class="{'bg-white dark:bg-dark-secondary border-b-2 -translate-y-3': scrollBackground, 'bg-light-primary dark:bg-dark-primary': !scrollBackground}">
      <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        <Link href="/" class="flex items-center">
            ...
            <span class="self-center text-2xl font-semibold whitespace-nowrap text-black dark:text-white transform duration-500 mt-1 ml-1" :class="{'text-base': scrollBackground, 'text-lg':!scrollBackground}">igitalDadKC</span>
        </Link>
        <button class="py-2 px-4 text-white rounded-lg dark:text-white bg-light-quatrenary dark:bg-slate-900 transform duration-500" :class="{'scale-80': scrollBackground, 'scale-110':!scrollBackground}" @click="toggleDark()">
